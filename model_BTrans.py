...
class TransformerContext(nn.Module):
    def __init__(self, config, obj_classes, rel_classes, in_channels):
        ...
        self.fusion_obj_fc = nn.Linear(self.hidden_dim, self.hidden_dim)
        self.fusion_rel_fc = nn.Linear(self.hidden_dim, self.hidden_dim)
    ...
    def forward(self, roi_features, proposals, logger=None):
        ...
        # =============== Bidirectional Transformer for Object Context Modeling ================== #
        re_obj_pre_rep = torch.flip(obj_pre_rep,dims=[0])
        re_num_objs = list(reversed(num_objs))
        re_obj_feats = self.context_obj(re_obj_pre_rep, re_num_objs)

        if obj_feats.shape == re_obj_feats.shape:
            # obj_feats = obj_feats + re_obj_feats
            obj_feats = obj_feats + torch.flip(re_obj_feats,dims=[0])
            obj_feats = self.fusion_obj_fc(obj_feats)
        ...
        # =============== Bidirectional Transformer for Relation Context Modeling ================== #
        re_edge_pre_rep = torch.flip(edge_pre_rep, dims=[0])
        re_edge_ctx = self.context_edge(re_edge_pre_rep, re_num_objs)

        if edge_ctx.shape == re_edge_ctx.shape:
            # obj_feats = obj_feats + re_obj_feats
            edge_ctx = edge_ctx + torch.flip(re_edge_ctx, dims=[0])
            edge_ctx = self.fusion_rel_fc(edge_ctx)
