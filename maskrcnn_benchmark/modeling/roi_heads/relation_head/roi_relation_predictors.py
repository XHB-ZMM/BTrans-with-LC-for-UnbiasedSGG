...
from .model_BTrans import BiTransformerContext
from .utils_relation import layer_init, get_box_info, get_box_pair_info
from maskrcnn_benchmark.data import get_dataset_statistics
import pdb
from .BCLloss import BalSCL
from operator import add
from functools import reduce

@registry.ROI_RELATION_PREDICTOR.register("BiTransformerPredictor")
class BiTransformerPredictor(nn.Module):
    def __init__(self, config, in_channels):
        ...
        # module construct
        self.context_layer = BiTransformerContext(config, obj_classes, rel_classes, in_channels)
        ...
        # =========================== BCL Work ================================= #
        self.cls_num_list = [1,
                             4445, 143, 159, 342, 405, 1202, 1043, 8521, 546, 348,
                             984, 322, 373, 354, 4, 659, 141, 134, 523, 48394,
                             6664, 15249, 2629, 435, 481, 212, 79, 181, 12285, 24596,
                             79428, 246, 675, 120, 471, 343, 50, 2243, 22, 3201,
                             1582, 245, 2983, 305, 189, 898, 301, 32257, 3398, 8539]

        self.soft_factor = config.SOLVER.SOFTENING_FACTOR
        self.scaling_factor = config.SOLVER.SCALING_FACTOR
        # =========================== BCL Work ================================= #
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        if self.training:
            ...
            # pdb.set_trace()
            soft_factor = self.soft_factor
            tau = self.scaling_factor
            cls_num_list = torch.cuda.FloatTensor(self.cls_num_list)
            cls_p_list = soft_factor * cls_num_list / cls_num_list.sum()
            m_list = tau * torch.log(cls_p_list)
            m_list = m_list.view(1, -1)
            # 谓词类别logits更新，是否只需要更新带有非0GT的logits有待商榷
            # 答案是：不需要额外处理，因为就算更新了0GT的，算loss时也会被乘以0
            rel_dists = rel_dists[:, :] + m_list[:, :]
        # =========================== BCL Work ================================= #
...
class MotifPredictor(nn.Module):
    def __init__(self, config, in_channels):
        ...
        self.cls_num_list = [1,
                             ...
                             79428, 246, 675, 120, 471, 343, 50, 2243, 22, 3201,
                             1582, 245, 2983, 305, 189, 898, 301, 32257, 3398, 8539]
        self.dim_in = self.pooling_dim  # 4096
        self.feat_dim = self.pooling_dim
        self.dim_hidden = int(self.pooling_dim / 2)
        self.criterion_scl = BalSCL(self.cls_num_list, 1)
        self.head_fc = nn.Sequential(nn.Linear(self.dim_in, self.dim_hidden),
                                     nn.BatchNorm1d(self.dim_hidden),
                                     nn.ReLU(inplace=True),
                                     nn.Linear(self.dim_hidden, self.feat_dim)
                                     )
        self.prototype_compress = nn.Linear(self.pooling_dim, self.num_rel_cls, bias=True)
        self.head = nn.Sequential(nn.Linear(self.dim_in, self.dim_hidden),
                                  nn.BatchNorm1d(self.dim_hidden),
                                  nn.ReLU(inplace=True),
                                  nn.Linear(self.dim_hidden, self.feat_dim)
                                  )
        self.v1_fc = nn.Sequential(nn.Linear(self.dim_in, self.dim_hidden),
                                   nn.Linear(self.dim_hidden, self.feat_dim),
                                   )
        self.v2_fc = nn.Sequential(nn.Linear(self.dim_in, self.dim_hidden),
                                   nn.Linear(self.dim_hidden, self.feat_dim),
                                   )
        self.device = torch.device(config.MODEL.DEVICE)
        self.dataset_logits_compensation = True # 采用是整个数据集还是每个batch的类频率
        # =========================== BCL Work ================================= #
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        # ===================== Complement & Averaging ======================== #
        # if self.training:
        #     rel_labes_list = reduce(add, [rel_labels[i].cpu().numpy().tolist() for i in range(len(rel_labels))])
        #     targets_centers = torch.arange(len(self.cls_num_list), device=self.device).view(-1, 1)
        #     targets = torch.Tensor([e for i, e in enumerate(rel_labes_list) if e != 0]).to(self.device).long()
        #     targets = torch.cat([targets.view(-1, 1), targets_centers], dim=0)
        #     batch_per_cls_count = torch.eye(len(self.cls_num_list))[targets].sum(dim=0).squeeze().long()
        #
        #     # 类补足的实现
        #     prototype = F.normalize(self.head_fc(self.rel_compress.weight), dim=1)
        #     prototype = prototype[:self.num_rel_cls]
        #     prototype_logits = self.rel_compress(prototype)
        #
        #     # pdb.set_trace()
        #     # Optimization with Logit Compensation
        #     soft_factor = 10
        ...
        #     # 谓词类别logits更新，是否只需要更新带有非0GT的logits有待商榷
        #     # 答案是：不需要额外处理，因为就算更新了0GT的，算loss时也会被乘以0
        #     prototype_logits = prototype_logits[:, :] + m_list[:, :]
        #
        #
        #     rel_dists = torch.cat([rel_dists,prototype_logits],dim=0)
        #     num_rels.append(int(51)) #类似于添加一张图像，里面有51个关系
        #     rel_labels.append(targets_centers.view(-1)) # gt也需要更新
        #
        #     # 类平均
        #     # # 获得前景谓词类对应的特征
        #     # non_zero_gt_rel_class_index = [i for i, e in enumerate(rel_labes_list) if e != 0]
        #     # features = prod_rep[non_zero_gt_rel_class_index]

        # ===================== Complement & Averaging ======================== #


        # =========================== BCL Work ================================= #
        # rel_dists 其实就是每个谓词类的logits，prod_rep就是每个谓词类的特征
        # self.rel_compress其实就是那个分类的权重的来源
        # rel_labels其实就是targets
        # cls_num_list要提前弄好，这个是数据集中所有类别的数量
        # scl_loss = 0
        # if self.training:
        #     # 先得到centers
        #     centers_logits = F.normalize(self.head_fc(self.rel_compress.weight), dim=1)
        #     centers_logits = centers_logits[:self.num_rel_cls]
        #     # pdb.set_trace()
        #
        #     # # 这里选择重复两遍谓词类特征，重复2个batch的
        #     # features = torch.cat([prod_rep.unsqueeze(1), prod_rep.unsqueeze(1)], dim=1)
        #     # # targets是一个一维的向量，由rel-labels合并而成
        #     # targets = reduce(add,[rel_labels[i].cpu().numpy().tolist() for i in range(len(rel_labels))])
        #     # targets = torch.Tensor(targets).to(self.device).long() # torch.Size([xxx]) 一个batch里谓词类别的数量
        #
        #     # 但我觉得，应该只收集具有非0-GT的特征和targets
        #     rel_labes_list = reduce(add, [rel_labels[i].cpu().numpy().tolist() for i in range(len(rel_labels))])
        #     non_zero_gt_rel_class_index = [i for i, e in enumerate(rel_labes_list) if e != 0]
        #     prod_rep = F.normalize(self.head(prod_rep), dim=1)
        #     f1 = self.v1_fc(prod_rep)
        #     f2 = self.v2_fc(prod_rep)
        #     features = torch.cat([f1[non_zero_gt_rel_class_index].unsqueeze(1),
        #                           f2[non_zero_gt_rel_class_index].unsqueeze(1)], dim=1)
        #     targets = torch.Tensor([e for i, e in enumerate(rel_labes_list) if e != 0]).to(self.device).long()
        #
        #     scl_loss = self.criterion_scl(centers_logits, features, targets)
        #     scl_loss = scl_loss * 0.01
        #
            # # Optimization with Logit Compensation
            # soft_factor = 10
            # tau = 1
        ...
        # =========================== Dataset Logit Compensation =================== #
        if self.training and self.dataset_logits_compensation:
            # Optimization with Logit Compensation
            soft_factor = 10
            tau = 1
            cls_num_list = torch.cuda.FloatTensor(self.cls_num_list)
        ...
        # =========================== Batch Logit Compensation =================== #
        if self.training and not self.dataset_logits_compensation:
            # 首先获得当前batch中出现谓词类别的个数
            # pdb.set_trace()
            batch_soft_factor = 1
            batch_tau = 1
            rel_labes_list = reduce(add, [rel_labels[i].cpu().numpy().tolist() for i in range(len(rel_labels))])
            targets = torch.Tensor([e for i, e in enumerate(rel_labes_list) if e != 0]).to(self.device).long().view(-1,1)
            batch_per_cls_count = torch.eye(len(self.cls_num_list))[targets].sum(dim=0).squeeze().long()
            batch_p_cls_list = batch_soft_factor * batch_per_cls_count / batch_per_cls_count.sum()
            batch_p_cls_list = torch.Tensor([batch_p_cls_list[i] if e != 0 else 1 for i, e in enumerate(batch_per_cls_count)]).to(self.device)
            m_list = batch_tau * torch.log(batch_p_cls_list)
            m_list = m_list.view(1, -1)
            rel_dists = rel_dists[:, :] + m_list[:, :]
        ...
        if self.attribute_on:
            ...
            return obj_dists, rel_dists, add_losses
            # return obj_dists, rel_dists, add_losses, scl_loss,rel_labels # 返回值也要遵循BCL的机理
