...
def compute_on_dataset(model, data_loader, device, synchronize_gather=True, timer=None):
    ...
    for _, batch in enumerate(tqdm(data_loader)):
        ...
        # if all(elem in targets[0].get_field("relation").view(-1).tolist() for elem in [31, 40]):
        #     on_max_value, max_index = torch.max(output[0].get_field("pred_rel_scores")[:, 31], dim=0)
        #     siting_on_max_value = output[0].get_field("pred_rel_scores")[max_index][40]
        #     print("GT = on时, on的置信度为：{}，此时sitiing on 的置信度为：{}".format(on_max_value,siting_on_max_value))
        #
        #
        #     siting_on_max_value_2, max_index_2 = torch.max(output[0].get_field("pred_rel_scores")[:, 40], dim=0)
        #     on_max_value_2 = output[0].get_field("pred_rel_scores")[max_index_2][31]
        #     print("GT = sitiing on时, on的置信度为：{}，此时sitiing on 的置信度为：{}".format(on_max_value_2, siting_on_max_value_2))
        #     print("==========================================================================")

            # pdb.set_trace()

        #output[0].get_field("pred_rel_scores")[24]
        #targets[0].get_field("relation")
        #output[0].get_field("rel_pair_idxs")

        # max_value, max_index = torch.max(output[0].get_field("pred_rel_scores")[:,40], dim=0)
        # output[0].get_field("pred_rel_scores")[max_index][38]
